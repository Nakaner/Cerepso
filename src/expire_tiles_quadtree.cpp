...
void ExpireTilesQuadtree::expire_from_coord_sequence(const std::vector<osmium::Location>& locations) {
    ...
    //HARDCODED 20 km
    if (segment_length(locations.front().lon(), locations.front().lat(), locations.back().lon(),
            locations.back().lat()) > 20000) {
        // If the line crosses 180th meridian, following expression will return false.
        if (std::abs(locations.front().lon() - locations.back().lon()) > 1) {
...
void ExpireTilesQuadtree::expire_from_coord_sequence(const osmium::NodeRef* begin, const osmium::NodeRef* end) {
    // check if distance between beginning and end is too large
    // We don't calculate the exact distance by summing up all segments.
    // This might return wrong results for U-shaped lines but it's faster.
    //TODO check if exact calculation based on shere is too slow.
    //HARDCODED 20 km
    const osmium::NodeRef* back = end;
    --back;
    if (segment_length(begin->lon(), begin->lat(), back->lon(), back->lat()) > 20000) {
        // If the line crosses 180th meridian, following expression will return false.
        if (std::abs(begin->lon() - end->lon()) > 1) {
            // expire only the tiles where the nodes are, not all tiles intersected by the line
