...
namespace test_utils {
    ...
    osmium::Node& create_new_node(osmium::memory::Buffer& buffer, const osmium::object_id_type id, const double lon, const double lat,
            const tagmap& tags) {
    ...
    osmium::Node& create_new_node_from_node_ref(osmium::memory::Buffer& buffer, const osmium::NodeRef& node_ref,
            const tagmap& tags) {
        return create_new_node(buffer, node_ref.ref(), node_ref.lon(), node_ref.lat(), tags);
    }
